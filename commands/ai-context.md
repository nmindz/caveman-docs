...
argument-hint: "<absolute-path | repo-name> [--repo-type=service|library|cli|mobile|infra]"
...
Post-agent artifact checks run in parallel.
Repo inspection delegated to agent — never done by router.
...
**If agent fails mid-run:** stop, report which artifacts were touched and the failure message. Do not synthesize files from agent output text. Re-invoke after inspecting failure.

## Verify on disk

Run in parallel after agent completes:
...
- Developer reviews diff and commits manually (whatever VCS).
- Re-run safe — idempotent.
- Monorepos: re-run per sub-project root.
