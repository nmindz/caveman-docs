...
AI documentation compression plugin for Claude Code and OpenCode. Enforces caveman-style compression when writing or generating AI-oriented docs (AGENTS.md, CLAUDE.md, docs/agents/_.md, SKILL.md, agents/_.md).
...
TypeScript plugin (`plugins/caveman-docs/opencode/caveman-docs.ts`) hooks `session.start` and `message.updated`:

| Hook              | When               | What                                                                  |
| ----------------- | ------------------ | --------------------------------------------------------------------- |
| `session.start`   | Session start      | Reads flag/env/config, injects mode-scoped rules into AGENTS.md block |
| `message.updated` | Every user message | Regex-matches activation/deactivation phrases, writes flag file       |

Slash commands shipped to `$CONFIG/command/`. Worker agent shipped to `$CONFIG/agent/doc-compressor.md`.
...
1. Flag file `~/.claude/.caveman-docs-active` (Claude Code) or `$CONFIG/.caveman-docs-active` (OpenCode), set by `/caveman-docs <level>`
...
```powershell
git clone <this-repo> $HOME\Projects\caveman-docs
cd $HOME\Projects\caveman-docs
hooks\opencode\install.ps1
```

Installs into `$CONFIG` (resolved as `$OPENCODE_CONFIG_DIR` → `$XDG_CONFIG_HOME/opencode` → `~/.config/opencode` → `$APPDATA\opencode`):

| Path                                 | Purpose                                                        |
| ------------------------------------ | -------------------------------------------------------------- |
| `$CONFIG/plugin/caveman-docs.ts`     | Hook plugin                                                    |
| `$CONFIG/command/caveman-docs*.md`   | 5 slash commands                                               |
| `$CONFIG/agent/doc-compressor.md`    | Worker agent                                                   |
| `$CONFIG/AGENTS.md` (block injected) | Activation rules between `<!-- BEGIN CAVEMAN-DOCS -->` markers |
...
```bash
bash hooks/opencode/uninstall.sh
```

```powershell
hooks\opencode\uninstall.ps1
```

## Platform

- Claude Code CLI (macOS/Linux)
- OpenCode (macOS/Linux/Windows)
